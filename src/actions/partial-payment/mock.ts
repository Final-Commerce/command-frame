...
export const mockPartialPayment: PartialPayment = async (
  ...
  // Contract (FI-6992): fixed `amount` arrives in integer MINOR units, the
  // same scale as the cart totals; percent (0-100) is of the remaining total.
  const charge = params?.isPercent ? Math.round((remaining * raw) / 100) : Math.round(raw);
