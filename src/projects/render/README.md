...
Start a refund flow for a given `orderId` (sets active order, opens refund UI, seeds selection). To observe refund selection state, subscribe to the [`refunds`](https://github.com/Final-Commerce/command-frame/blob/main/src/pubsub/topics/refunds/README.md) topic (`set-active-refund` and `get-active-refund` events carry `CFActiveRefundDetails`).
