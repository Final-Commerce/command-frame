...
Loads an order and initializes refund selection state in the host (opens refund UI as applicable). There is no `getActiveRefund` command; read [`CFActiveRefundDetails`](https://github.com/Final-Commerce/command-frame/blob/main/src/types/README.md#cfactiverefunddetails) from pub/sub on topic `refunds` (events `set-active-refund` / `get-active-refund`) when the host publishes updates.
