...
`triggerWebhook` accepts only the fields in [`TriggerWebhookParams`](../actions/trigger-webhook/types.ts) (there is no `payload` property). Use `presetData` / `presetType` for host-shaped presets, or encode a custom URL as a literal inside the hook.
