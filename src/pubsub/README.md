...
- Topic IDs are kebab-case strings from host topic definitions (for example `'custom-tables'`). Event type strings are also kebab-case (for example `'payment-done'` on the `'payments'` topic).
