...
**Constraints:**
- Topic IDs are kebab-case strings from host topic definitions (for example: `'cart'`, `'custom-tables'`, `'payments'`). Event type IDs are separate (for example `'payment-done'` on topic `'payments'`).
