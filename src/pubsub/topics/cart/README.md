...
The `cart` topic provides events related to cart operations. Subscribe to this topic to receive real-time notifications when the cart is created, customers are assigned or unassigned, products are added/updated/removed, cart- or line-level discounts and fees change, or product notes change.

The authoritative list of wire event IDs is [`cart/index.ts`](./index.ts) (`cartTopic.eventTypes`).
...
const subscriptionId = topics.subscribe("cart", (event: TopicEvent) => {
    switch (event.type) {
        ...
        case "product-note-added":
            console.log("Line note added:", event.data.note);
            break;
        case "product-note-removed":
            console.log("Line note removed");
            break;
    }
});
```
