// All host calls go through `renderClient` — the singleton kaching claims and
// serves in-process. The standalone `dist/actions/*/action` functions dispatch
// via the separate `commandFrameClient` singleton, which the in-bundle runtime
// never serves, so they dead-end in the base mock registry (`_source: "mock"`).
import { useState, useEffect } from 'react';
import { topics, renderClient } from '@final-commerce/command-frame';
    ...
    <div className="section-content">
      <CommandSection title="Can Transition">
        <p className="section-description">
          Check whether an order can transition to a target state pair. Leave Order ID blank to evaluate against a new
          (null) order.
        </p>
          ...
          onClick={async () => {
            ...
            try {
              ...
              const { result, source } = await callWithFallback(
                (p) => renderClient.canTransition(p),
                canTransitionMock,
                params,
              );
        ...
        {ctResponse && (
          <JsonViewer
            data={ctResponse}
            title={
              ctResponse.includes('"allowed": true') ? 'Allowed' : ctResponse.startsWith('Error') ? 'Error' : 'Blocked'
            }
          />
        )}
      </CommandSection>
      ...
      <CommandSection title="Get Available Transitions">
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              const { result, source } = await callWithFallback(
                (p) => renderClient.getAvailableTransitions(p),
        ...
        {atResponse && (
          <JsonViewer data={atResponse} title={atResponse.startsWith('Error') ? 'Error' : 'Transitions'} />
        )}
      </CommandSection>

      <CommandSection title="Apply Transition">
        <p className="section-description">
          Move an order's fulfillment state. The payment axis is never client-settable. The host validates the
          transition before applying it — a blocked result returns the guard name and reason without changing the order.
          Leave Order ID empty to target the active order — the host materializes one from the live cart if none exists
          (like park, without park semantics).
        </p>
        <div className="form-group">
          <div className="form-field">
            ...
            <input
              type="text"
              value={apOrderId}
              onChange={(e) => setApOrderId(e.target.value)}
              placeholder="order_1001"
            />
          </div>
          <div className="form-field">
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              const { result, source } = await callWithFallback(
                (p) => renderClient.applyTransition(p),
                applyTransitionMock,
                params,
              );
              const output = { ...result, _source: source };
        ...
        {apResponse && (
          <JsonViewer
            data={apResponse}
            title={
              apResponse.includes('"allowed": true')
                ? 'Transition Applied'
                : apResponse.startsWith('Error')
                  ? 'Error'
                  : 'Blocked'
            }
          />
      ...
      <CommandSection title="Payment with Checkout Fulfillment Target">
        <p className="section-description">
          Trigger a cash payment with an explicit <code>checkoutFulfillmentTarget</code>. This override tells Render
          what fulfillment state the order should land in after full payment (e.g. <code>in_progress</code> for
          restaurants, <code>fulfilled</code> for retail).
          <br />
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              const cart = await renderClient.getCurrentCart();
              const balanceDue = cart.cart?.amountToBeCharged ?? cart.cart?.total ?? 0;
              const result = await renderClient.cashPayment({
                amount: balanceDue,
        ...
        {cftResponse && (
          <JsonViewer data={cftResponse} title={cftResponse.startsWith('Error') ? 'Error' : 'Payment Result'} />
        )}
      </CommandSection>

      <CommandSection title="State Transition Events">
        <p className="section-description">
          Subscribe to <code>state-transition-completed</code> and <code>state-transition-blocked</code> events on the
          orders topic. Events appear here in real time when the host publishes them.
        </p>
        <button
