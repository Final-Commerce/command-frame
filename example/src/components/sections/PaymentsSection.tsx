    ...
    return (
        <div className="section-content">
            ...
            <CommandSection title="Cash Payment">
                ...
                {cashPaymentResponse && (
                    <JsonViewer data={cashPaymentResponse} title={cashPaymentResponse.startsWith("Error") ? "Error" : "Success"} />
                )}
            </CommandSection>

            {/* Tap to Pay */}
            <CommandSection title="Tap to Pay Payment">
                <p className="section-description">Initiates a Tap to Pay payment. Leave amount empty to use cart total.</p>
                <div className="form-group">
                    <div className="form-field">
                        <label>Amount (optional):</label>
                        <input
                ...
                {tapToPayResponse && <JsonViewer data={tapToPayResponse} title={tapToPayResponse.startsWith("Error") ? "Error" : "Success"} />}
            </CommandSection>

            {/* Terminal Payment */}
            <CommandSection title="Terminal Payment">
                <p className="section-description">Initiates a terminal payment. Leave amount empty to use cart total.</p>
                <div className="form-group">
                    <div className="form-field">
                        <label>Amount (optional):</label>
                        <input
            ...
            <CommandSection title="Redeem payment">
                <p className="section-description">
                    ...
                    <code>paymentType: &quot;redeem&quot;</code>. Requires cart with items when embedded in Render. <code>amount</code> is required
                    (in minor units; e.g. <code>500</code> = $5.00). If the amount is less than the cart balance, the host requires the cart to be in
                    split-payment mode already.
                </p>
                <div className="form-group">
                    <div className="form-field">
                    ...
                    <div className="form-field">
                        ...
                        <input type="text" value={redeemProcessor} onChange={e => setRedeemProcessor(e.target.value)} placeholder="giftCard" />
                    </div>
                    <div className="form-field">
                ...
                {redeemResponse && <JsonViewer data={redeemResponse} title={redeemResponse.startsWith("Error") ? "Error" : "Success"} />}
            </CommandSection>

            {/* Integration payment (Stripe-like) */}
            <CommandSection title="Integration payment">
                <p className="section-description">
                    Calls <code>integrationPayment</code> — for Stripe-style integrations. The extension processes the payment with its own provider,
                    then reports back so Render records the transaction + order. Wire <code>paymentType: &quot;integration&quot;</code>. Required:{" "}
                    <code>amount</code> (minor units) and <code>emvData</code> (typed object — host maps camelCase keys to canonical EMV keys and
                    JSON-serializes onto the order&apos;s <code>paymentMethod.emv</code>). If the amount is less than the cart balance, the host
                    requires split-payment mode to be active.
                </p>
                <div className="form-group">
                    <div className="form-field">
                        <label>Amount (required, minor units):</label>
                    ...
                    <div className="form-field">
                        ...
                        <input type="text" value={integrationEmvBrand} onChange={e => setIntegrationEmvBrand(e.target.value)} placeholder="Visa" />
                    </div>
                    <div className="form-field">
                    ...
                    <div className="form-field">
                        ...
                        <input type="text" value={integrationEmvExpiry} onChange={e => setIntegrationEmvExpiry(e.target.value)} placeholder="12/26" />
                    </div>
                    <div className="form-field">
                        <label>EMV — Country (optional):</label>
                        <input type="text" value={integrationEmvCountry} onChange={e => setIntegrationEmvCountry(e.target.value)} placeholder="US" />
                    </div>
                    <div className="form-field">
                        <label>EMV — Issuer (optional):</label>
                        <input type="text" value={integrationEmvIssuer} onChange={e => setIntegrationEmvIssuer(e.target.value)} placeholder="Chase" />
                    </div>
                    <div className="form-field">
                        <label>Processor (optional):</label>
                        <input
                    ...
                    <div className="form-field">
                        <label>Label (optional):</label>
                        <input type="text" value={integrationLabel} onChange={e => setIntegrationLabel(e.target.value)} placeholder="Visa ****4242" />
                    </div>
                    <div className="form-field">
                        <label>Extension ID (optional):</label>
                        <input
                ...
                {integrationResponse && (
                    <JsonViewer data={integrationResponse} title={integrationResponse.startsWith("Error") ? "Error" : "Success"} />
                )}
            </CommandSection>
            ...
            <CommandSection title="Extension payment (generic)">
                <p className="section-description">
                    Calls <code>extensionPayment</code> with a <code>paymentType</code>. For <code>redeem</code>, behavior matches Redeem payment
                    above. Other types will error until the host implements them.
                </p>
                <div className="form-group">
                    <div className="form-field">
                        <label>paymentType:</label>
                        <input type="text" value={extPaymentType} onChange={e => setExtPaymentType(e.target.value)} placeholder="redeem" />
                    </div>
                    <div className="form-field">
                        <label>Amount (optional):</label>
                        <input
                ...
                {extResponse && <JsonViewer data={extResponse} title={extResponse.startsWith("Error") ? "Error" : "Success"} />}
            </CommandSection>

            {/* Partial Payment */}
            <CommandSection title="Partial Payment">
                <p className="section-description">Initiates a partial/split payment. Can open UI or process with specified amount.</p>
                <div className="form-group">
                    <div className="form-field">
                        <label className="checkbox-label">
                            <input type="checkbox" checked={partialPaymentOpenUI} onChange={e => setPartialPaymentOpenUI(e.target.checked)} />
                ...
                {partialPaymentResponse && (
                    <JsonViewer data={partialPaymentResponse} title={partialPaymentResponse.startsWith("Error") ? "Error" : "Success"} />
                )}
            </CommandSection>
        </div>
    );
