    ...
    <div className="section-content">
      ...
      <CommandSection title="Get Cash Rounding Amount">
        <p className="section-description">
          Previews the company's cash rounding for an amount (integer minor units, e.g. 1577 = $15.77). Leave empty for
          the cart's balance due. No setting configured → returns the amount unchanged.
        </p>
      ...
      <CommandSection title="Cash Payment">
        <p className="section-description">
          Processes a cash payment. Amounts are integer MINOR units (1575 = $15.75). Amount is required — below the
          balance due it becomes a partial payment (split leg). Enter what the customer handed over as Tendered and the
          response returns the change (cash rounding included) — the flow owns the tender UI now ("Get Cash Rounding
          Amount" above pre-fills the rounded charge).
        </p>
        <div className="form-group">
          <div className="form-field">
      ...
      <CommandSection title="Tap to Pay Payment">
        ...
        {tapToPayResponse && (
          <JsonViewer data={tapToPayResponse} title={tapToPayResponse.startsWith('Error') ? 'Error' : 'Success'} />
        )}
      </CommandSection>
      ...
      <CommandSection title="Terminal Payment">
        ...
        {terminalResponse && (
          <JsonViewer data={terminalResponse} title={terminalResponse.startsWith('Error') ? 'Error' : 'Success'} />
        )}
      </CommandSection>
      ...
      <CommandSection title="Cloud Payment">
        <p className="section-description">
          Initiates a cloud (HTTP) terminal payment via <code>paymentType: "Cloud"</code>. Works on web. Leave amount
          empty to use cart total.
        </p>
        <div className="form-group">
          <div className="form-field">
            <label>Amount (optional):</label>
        ...
        {cloudResponse && (
          <JsonViewer data={cloudResponse} title={cloudResponse.startsWith('Error') ? 'Error' : 'Success'} />
        )}
      </CommandSection>
      ...
      <CommandSection title="Redeem payment">
        ...
        <div className="form-group">
          ...
          <div className="form-field">
            ...
            <input
              type="text"
              value={redeemLabel}
              onChange={(e) => setRedeemLabel(e.target.value)}
        ...
        {redeemResponse && (
          <JsonViewer data={redeemResponse} title={redeemResponse.startsWith('Error') ? 'Error' : 'Success'} />
        )}
      </CommandSection>
      ...
      <CommandSection title="Integration payment">
        <p className="section-description">
          ...
          the order&apos;s <code>paymentMethod.emv</code>). If the amount is less than the cart balance, the host
