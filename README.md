...
The library centers on **commands**, **pub/sub**, and **hooks** (table below). **Extension refunds** are a separate host-to-iframe protocol for redeem / gift-card reversals during POS refunds (see [Extension refund](./src/actions/extension-refund/README.md)).
...
| Kind                                       | Source                                                                                                                               | Notes                                                                                                                     |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| Domain models (`CFOrder`, `CFCustomer`, …) | [`src/CommonTypes.ts`](./src/CommonTypes.ts), [`src/common-types/`](./src/common-types/)                                             | Re-exported from the package root                                                                                         |
| Command params / responses                 | [`src/actions/<action>/types.ts`](./src/actions/), [`src/index.ts`](./src/index.ts)                                                  | Same names as `command.<camelCase>()` and client methods                                                                  |
| Pub/sub payloads                           | [`src/pubsub/topics/`](./src/pubsub/topics/), `TopicEventPayloadMap` in [`src/pubsub/topics/types.ts`](./src/pubsub/topics/types.ts) | Subscribe with kebab-case topic ids; map keys are camelCase — see [Pub/Sub](./src/pubsub/README.md#topic-id-mapping-note) |
| Hooks (iframe API)                         | [`src/hooks/types.ts`](./src/hooks/types.ts)                                                                                         | `HookFunction`, `HookRegisterOptions`; see [Hooks](./src/hooks/README.md)                                                 |
